...
# Loading Dataset
df = pd.read_excel("Quran_Project.xlsx")
...
if menu_id=='Environmental Search':
  ...
  """, unsafe_allow_html=True)
    ...
    for i in range(len(df["Arabic Verse"])):
      ...
      sc_html = sc_html + search_card_html(highlighted_verse,df["Arabic Chapter Title With Harakat"].iloc[i],df["Verse Nb."].iloc[i],find_concept_env(i),
      'لم تنجز بعد','لم تنجز بعد')
          ...
          if len(match)!=0:
            for i in range(len(match)):
              if environment_test(match,i)==True:
                ...
                "Yes",concept_heading="Environmental Concept(s): ",concept=find_concept(match,i),
                env_verse_heading="Verse with Environmental Words: ",env_verse= "Not Finished Yet")
              else:
                expander(highlight_literal(match, i), match["English Chapter Title"].iloc[i], match["eng_verse_nb"].iloc[i],match["arabic_verse"].iloc[i],
                "No")
        ...
        if len(quran_df_filtered) != 0:  
          for i in range(len(quran_df_filtered)):
            if environment_test(quran_df_filtered,i)==True:
              ...
              "Yes", concept_heading="Environmental Concept(s): ",concept=find_concept(quran_df_filtered,i),
              env_verse_heading="Verse with Environmental Words: ",env_verse="Not Finished Yet")
            else:
              expander(highlight_stem(quran_df_filtered, "normalized_verse_for_highlight", i, query_normalized_one_list), 
              quran_df_filtered["English Chapter Title"].iloc[i], quran_df_filtered["eng_verse_nb"].iloc[i], quran_df_filtered["arabic_verse"].iloc[i],
          ...
          if len(match)!=0:
            for i in range(len(match)):
              if environment_test(match,i)==True:
                ...
                "نعم",concept_heading="المفهوم البيئي: ",concept=find_concept(match,i),
        ...
        if len(quran_df_filtered) != 0:  
          for i in range(len(quran_df_filtered)):
            if environment_test(quran_df_filtered,i)==True:
              ...
              "نعم", concept_heading="المفهوم البيئي: ",concept=find_concept(quran_df_filtered,i),
...
<body>
<div class="card">
  <img src="https://media-exp1.licdn.com/dms/image/C4E03AQGYYhglXBeeSw/profile-displayphoto-shrink_800_800/0/1656601448671?e=1665619200&v=beta&t=brqx22c9QBcGVBDQB5aTuNgt2wuzMfyzKeSrXGjaMi4" alt="Mahdi Mohammad" style="width:100%; height:300px">
  <h1 style="font-size: 25px; color:green"><b>Mahdi Mohammad</b></h1>
  <p style="color:black" class="title">Graduate, MS. in Business<br> Analytics (MSBA)</p>
  <div style="margin: 24px 0;">
...
<body>
<div class="card">
  <img src="https://icons-for-free.com/download-icon-lady+user+woman+icon-1320166736788209986_512.png" alt=Mrs. Sara Moussalli style="width:100%; height:300px">
  <h1 style="font-size: 25px; color:green"><b>Mrs. Sara Moussalli</b></h1>
  <p style="color:black" class="title">Research Assistant, History<br> & Islamic Studies</p>
  <div style="margin: 24px 0;">
